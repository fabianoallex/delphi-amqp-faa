...
MVP completo. **Publisher confirms** (`confirm.select`) implementados: `ConfirmSelect`, `Publish` retorna seq-no, `OnConfirm` (thread pool) + `WaitForConfirm`/`WaitForConfirms`. `Basic.Return` já é tratado (evento `OnBasicReturn`). **TLS (amqps://)** implementado via SChannel nativo do Windows (`src/AMQP.Transport.Tls.pas`, `UseTls`/`TlsVerifyPeer`) — sem OpenSSL/DLLs externas, aditivo ao transporte plain. **Connection.Blocked/Unblocked** (extensão RabbitMQ) tratados: capability anunciada em client-properties + eventos `OnBlocked`/`OnUnblocked` (thread pool). **Queue.Unbind** e **Exchange.Bind/Exchange.Unbind** implementados (`UnbindQueue`/`BindExchange`/`UnbindExchange`; unbind remove o bind equivalente da topologia de recovery). **Reenvio automático de publishes não confirmados** (`RepublishUnconfirmedOnReconnect`, opt-in): re-publica na reconexão o que ficou sem confirmação, com seq-nos novos (at-least-once). **Adapter**: escape hatch `IDelphiAmqpFaaNative` (acesso ao canal/conexão nativos via `Supports`) + TLS por convenção de porta (5671). Fixture de integração TLS (`tests/Integration/AMQP.TlsIntegrationTests.pas`) com skip gracioso quando o broker TLS não está no ar. Próximos passos possíveis (não bloqueiam uso): transações (`tx.*` — descartado por decisão de design, ver README), mTLS/client-cert (exige `crypt32`), campos de TLS em `TMessagingConfig` no infra, recuperação de topologia para filas com nome gerado pelo servidor.
