...
MVP completo. **Publisher confirms** (`confirm.select`) implementados: `ConfirmSelect`, `Publish` retorna seq-no, `OnConfirm` (thread pool) + `WaitForConfirm`/`WaitForConfirms`. `Basic.Return` já é tratado (evento `OnBasicReturn`). **TLS (amqps://)** implementado via SChannel nativo do Windows (`src/AMQP.Transport.Tls.pas`, `UseTls`/`TlsVerifyPeer`) — sem OpenSSL/DLLs externas, aditivo ao transporte plain. **Connection.Blocked/Unblocked** (extensão RabbitMQ) tratados: capability anunciada em client-properties + eventos `OnBlocked`/`OnUnblocked` (thread pool). **Queue.Unbind** implementado (`UnbindQueue`, remove o bind equivalente da topologia de recovery). Fixture de integração TLS (`tests/Integration/AMQP.TlsIntegrationTests.pas`) com skip gracioso quando o broker TLS não está no ar. Próximos passos possíveis (não bloqueiam uso): transações (`tx.*` — descartado por decisão de design, ver README), mTLS/client-cert, exposição de TLS no adapter `Messaging.Adapters.DelphiAmqpFaa`, `Exchange.Bind`/`Exchange.Unbind` (exchange-a-exchange), recuperação de topologia para filas com nome gerado pelo servidor, reenvio automático de publishes não confirmados após reconexão.
