...
begin
  ...
  FChan.Consume(LQueue,
    ...
    begin
      LCur := TInterlocked.Increment(FCurrent);
      // mesma atualização de pico do teste de concorrência; aqui esperamos
      // que nunca passe de 1 (worker dedicado é sequencial).
      repeat
        LOldPeak := FPeak;
        if LCur <= LOldPeak then
          Break;
      ...
      // Dá tempo de sobra para outro worker do pool nativo sobrepor, se o
