...
destructor TAMQPFieldTable.Destroy;
  ...
  begin
    ...
    begin
      LArr := AValue.AsType<TArray<TValue>>;
...
begin
  LLen := ReadOctet;
  Result := TEncoding.UTF8.GetString(ReadRaw(LLen));
end;
...
begin
  LLen := ReadLongUInt;
  Result := TEncoding.UTF8.GetString(ReadRaw(Integer(LLen)));
end;
  ...
  case LTag of
    ...
    FV_DECIMAL:
      begin
        ...
        Result := LUnscaled / Power10(LScale);
      end;
    ...
    FV_ARRAY:
      begin
        ...
        Result := TValue.From<TArray<TValue>>(LArr);
      end;
  else
